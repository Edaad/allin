...
const sendInvitations = async (req, res) => {
	...
	try {
		// Check if valid game and inviter
		const [game, inviter] = await Promise.all([
		...
		// Track results for better debugging
		const results = {
			invited: [],
			errors: [],
			alreadyInvited: [],
		...
		for (const inviteeId of inviteeIds) {
			try {
				...
				if (!invitee) {
					...
					console.error(`Invitee not found: ${inviteeId}`);
					continue;
				}
				...
				if (existingPlayer) {
					results.alreadyInvited.push(inviteeId);
					console.log(
						`User ${inviteeId} already has an invitation for game ${gameId}`
					);
					continue;
				}

				// Create new player with pending status
				...
				try {
					await notificationService.notifyGameInvitationReceived(
						inviteeId,
						inviterId,
						gameId
					);
				...
				results.invited.push(inviteeId);
				console.log(
					`Successfully invited user ${inviteeId} to game ${gameId}`
				);
			} catch (error) {
				results.errors.push({ id: inviteeId, reason: error.message });
...
const cancelInvitation = async (req, res) => {
	try {
		...
		if (game.host_id.toString() !== inviterId) {
			return res
				.status(403)
				.send({ message: "Only the host can cancel invitations." });
		}
...
const getGamePlayers = async (req, res) => {
	try {
		...
		const players = await Player.find({ game_id: gameId }).populate(
			"user_id",
		...
		if (requesterId) {
			...
			if (game.host_id.toString() !== userId) {
				return res
					.status(403)
					.json({
						message: "Only the host can accept join requests.",
					});
			}
			...
			if (!player) {
				return res
					.status(404)
					.json({ message: "Join request not found." });
			}
			...
			if (acceptedPlayersCount < game.handed) {
				...
				try {
					await notificationService.notifyGameJoinAccepted(
						requesterId,
						userId,
						gameId
					);
			...
			if (acceptedPlayersCount < game.handed) {
				...
				try {
					await notificationService.notifyGameInvitationAccepted(
						game.host_id,
						userId,
						gameId
					);
...
const declineInvitation = async (req, res) => {
	try {
		...
		try {
			await notificationService.notifyGameInvitationDeclined(
				game.host_id,
				userId,
				gameId
			);
...
const getInvitationsForPlayer = async (req, res) => {
	try {
		...
		const validInvitations = invitations.filter(
			(invitation) => invitation.game_id != null
		);
	...
	try {
		...
		if (game.host_id.toString() !== inviterId && inviterId !== inviteeId) {
			return res
				...
				.json({
					message:
						"You are not authorized to remove this player from the game.",
				});
		}
		...
		if (!result) {
			return res
				.status(404)
				.json({ message: "Player not found in the game." });
		}

		// If host is removing player, notify the player
		if (game.host_id.toString() === inviterId && inviterId !== inviteeId) {
			try {
				await notificationService.notifyPlayerRemoved(
					inviteeId,
					inviterId,
					gameId
				);
		...
		// If player is removing themselves, notify the host
			...
			try {
				await notificationService.notifyPlayerLeft(
					game.host_id,
					inviteeId,
					gameId
				);
		...
		if (result.invitation_status === "accepted") {
			...
			if (acceptedPlayersCount < game.handed) {
				...
				if (waitlistedPlayer) {
					...
					return res.status(200).json({
						message:
							"Player removed and waitlisted player promoted.",
	...
	try {
		...
		if (!game.is_public) {
			return res
				...
				.json({
					message:
						"This game is private. You cannot request to join.",
				});
		}
		...
		// Create a new player record with appropiate status
		...
		res.status(201).json({
			...
			position: null, // Position will be determined later if on waitlist
		});
	} catch (err) {
...
const getGameJoinRequests = async (req, res) => {
	try {
		...
		if (game.host_id.toString() !== hostId) {
			return res
				.status(403)
				.json({ message: "Only the host can view join requests." });
		}
...
const rejectJoinRequest = async (req, res) => {
	try {
		...
		if (game.host_id.toString() !== hostId) {
			return res
				.status(403)
				.json({ message: "Only the host can reject join requests." });
		}
		...
		res.status(200).json({
			...
			request_type:
				result.invitation_status === "waitlist_requested"
					? "waitlist"
					: "standard",
		});
	} catch (err) {
...
const getWaitlistPosition = async (req, res) => {
	try {
		...
		if (!player) {
			return res
				.status(404)
				.json({ message: "Player not found on waitlist." });
		}
...
const getRequestedGames = async (req, res) => {
	try {
		...
		const formattedGames = games.map((game) => {
			...
			const playerInfo = playerStatusMap[gameId] || {
				status: "unknown",
