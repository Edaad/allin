	...
	try {
		...
		if (userId) {
			...
			const games = await Game.find(query)
				...
				.populate("group_id", "group_name") // Add this line to populate group details
			...
			const filteredGames = games
				.map((game) => {
					...
					gameObject.playerStatus = playerStatusMap[gameId] || "none";
					return gameObject;
				})
				.filter((game) => {
...
const deleteGame = async (req, res) => {
	try {
		...
		try {
			await notificationService.notifyGameDeleted(
				game._id,
				gameName,
				playerIds,
				hostId
