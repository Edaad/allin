...
const notifyGameCreated = async (userId, gameId) => {
    try {
        ...
        await createNotification({
            ...
            type: 'game_created',
            title: 'Game Created',
            message: `You've successfully created a new game: ${game.game_name}`,
            referenced_id: gameId,
            referenced_model: 'Game',
            link: `/dashboard/${userId}/host/game/${gameId}`
        });
    } catch (err) {
        console.error('Error in notifyGameCreated:', err);
    }
};
...
const notifyGameJoinRejected = async (playerId, hostId, gameId, reason) => {
    try {
        ...
        await createNotification({
            ...
            message: `Your request to join ${game.game_name} was declined. Reason: ${reason || 'No reason provided'}`,
            referenced_id: gameId,
            referenced_model: 'Game',
...
const notifyGameInvitationSent = async (hostId, inviteeCount, gameId) => {
    try {
        ...
        await createNotification({
            ...
            message: `You've invited ${inviteeCount} player${inviteeCount !== 1 ? 's' : ''} to join your game: ${game.game_name}`,
            referenced_id: gameId,
            referenced_model: 'Game',
            link: `/dashboard/${hostId}/host/game/${gameId}`
        });
    } catch (err) {
        console.error('Error in notifyGameInvitationSent:', err);
    }
};
...
const notifyGroupJoinRejected = async (userId, adminId, groupId, reason) => {
    try {
        ...
        await createNotification({
            ...
            message: `Your request to join ${group.group_name} was declined. Reason: ${reason || 'No reason provided'}`,
            referenced_id: groupId,
            referenced_model: 'Group',
...
const notifyGroupInvitationSent = async (adminId, inviteeCount, groupId) => {
    try {
        ...
        await createNotification({
            ...
            message: `You've invited ${inviteeCount} user${inviteeCount !== 1 ? 's' : ''} to join your group: ${group.group_name}`,
            referenced_id: groupId,
            referenced_model: 'Group',
            link: `/dashboard/${adminId}/groups/${groupId}`
        });
