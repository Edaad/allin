...
export function ProfileManagement() {
		...
		const fetchProfile = async () => {
			...
			try {
				...
				const response = await axios.get(
					`${process.env.REACT_APP_API_URL}/profiles/user/${userId}`
				);
				...
				setForm({
					...
					poker_preferences: {
						preferred_blinds:
							response.data.poker_preferences?.preferred_blinds ||
							[],
						availability: {
							weekdays:
								response.data.poker_preferences?.availability
									?.weekdays || false,
							weeknights:
								response.data.poker_preferences?.availability
									?.weeknights || false,
		...
		setForm((prev) => {
			...
			if (checked) {
				...
				return {
					...
					poker_preferences: {
						...prev.poker_preferences,
						preferred_blinds: currentBlinds.filter(
							(blind) => blind !== value
						),
					},
				};
	...
	const handleSubmit = async (e) => {
		...
		try {
			...
			const response = await axios.put(
				`${process.env.REACT_APP_API_URL}/profiles/${userId}`,
				{
					...form,
	...
	const handleImageUpload = async (type, imageUrl) => {
		try {
			...
			const response = await axios.patch(
				`${process.env.REACT_APP_API_URL}/profiles/${userId}/image`,
				{
					imageUrl,
	...
	const handleCancel = () => {
		...
		setForm({
			...
			poker_preferences: {
				preferred_blinds:
					profile.poker_preferences?.preferred_blinds || [],
				availability: {
					weekdays:
							...
							<div className="profile-banner">
								...
								{profile?.banner_image && (
									<button
										...
										onClick={() => {
											const url = window.prompt(
												"Enter new image URL for banner:"
											);
											if (url)
												handleImageUpload(
													"banner",
									...
									<div className="blinds-selection">
										...
										<div className="blinds-options">
											{BLINDS_OPTIONS.map((blind) => (
												<label
													key={blind}
													className="blinds-checkbox-label"
												>
													<input
														type="checkbox"
														name={`blind-${blind}`}
														value={blind}
														checked={form.poker_preferences.preferred_blinds.includes(
															blind
														)}
														onChange={
															handleBlindsChange
									...
									<div className="details-section">
										...
										<div className="hosted-games">
											<table className="games-table">
												...
												<tbody>
													{profile.hostedGames.map(
														(game) => (
															<tr
																key={game._id}
																onClick={() =>
																	navigate(
																		`/dashboard/${userId}/games/game/${game._id}`
																	)
																}
															>
																<td>
																...
																<td>
																	<span
																		className={`status-tag ${game.game_status}`}
																	>
																		{
																			game.game_status
																		}
